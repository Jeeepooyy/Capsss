...
$year = isset($_GET['year']) ? $_GET['year'] : date("Y"); // Get the selected year or use current year
$conn = new mysqli("localhost", "root", "", "hcpms") or die(mysqli_error());

// Fetch monthly patient counts for the chart based on complaint date
$patient_counts = [];
for ($month = 1; $month <= 12; $month++) {
	$query = $conn->query("SELECT COUNT(*) as total 
                           FROM `complaints` 
                           WHERE `section` IN ('Maternity', 'Prenatal') 
                           AND YEAR(`date`) = '$year' 
                           AND MONTH(`date`) = '$month'") or die(mysqli_error());
	$result = $query->fetch_array();
	$patient_counts[] = $result['total'];
}


// Fetch age distribution for Maternity patients
$age_ranges = [
	"<20" => "age BETWEEN 0 AND 20",
...
foreach ($age_ranges as $range => $condition) {
	$query = $conn->query("
        SELECT COUNT(DISTINCT complaints.itr_no) as total 
        FROM `itr` 
        INNER JOIN `complaints` 
        ON itr.itr_no = complaints.itr_no 
        WHERE complaints.section IN ('Maternity', 'Prenatal') 
        AND YEAR(complaints.date) = '$year'AND $condition
...
// Fetch gender distribution for Maternity patients
$query = $conn->query("
    SELECT gender, COUNT(DISTINCT complaints.itr_no) as total 
    FROM `itr` 
    INNER JOIN `complaints` 
    ON itr.itr_no = complaints.itr_no 
    WHERE complaints.section IN ('Maternity', 'Prenatal') 
	AND YEAR(complaints.date) = '$year'
    GROUP BY gender
") or die(mysqli_error($conn));
...
if (array_sum($patient_counts) === 0) {
	$recommendations[] = "No patient records for Maternity and Prenatal this year. Consider outreach programs to raise awareness.";
} else {
...
$recommendations[] = "Majority of patients belong to the age group $largest_age_group. Consider tailoring services for this demographic.";
	...
	<script type="text/javascript">
		window.onload = function() {
			...
			$(".chartContainer").CanvasJSChart({
				...
				theme: "light2",
				title: {
					text: "Monthly Maternity and Prenatal Patient Population <?php echo $year ?>"
				},
				axisY: {
			...
			$(".ageDistributionContainer").CanvasJSChart({
				...
				theme: "light2",
				title: {
					text: "Age Distribution of Maternity and Prenatal Patients"
				},
				axisY: {
			...
			$(".genderDistributionContainer").CanvasJSChart({
				...
				theme: "light2",
				title: {
					text: "Gender Distribution of Maternity and Prenatal Patients"
				},
				data: [{
					type: "pie",
					showInLegend: true,
			...
			<div id="target">
				...
				<table id="table" cellspacing="0" class="display">
					...
					<tbody>
						<?php
						$conn = new mysqli("localhost", "root", "", "hcpms") or die(mysqli_error());
						$query = $conn->query("SELECT itr.itr_no, itr.firstname, itr.lastname, itr.age, itr.gender, itr.address, complaints.date AS complaint_date FROM complaints INNER JOIN itr ON complaints.itr_no = itr.itr_no WHERE complaints.section = 'Maternity' GROUP BY itr.itr_no ORDER BY complaints.date DESC") or die(mysqli_error());
			...
			<div id="target1">
				...
				<table id="table1" cellspacing="0" class="display">
					...
					<tbody>
						<?php
						$conn = new mysqli("localhost", "root", "", "hcpms") or die(mysqli_error());
						$query = $conn->query("SELECT itr.itr_no, itr.firstname, itr.lastname, itr.age, itr.gender, itr.address, complaints.date AS complaint_date FROM complaints INNER JOIN itr ON complaints.itr_no = itr.itr_no WHERE complaints.section = 'Prenatal' GROUP BY itr.itr_no ORDER BY complaints.date DESC") or die(mysqli_error());
						while ($fetch = $query->fetch_array()) {
							$formatted_date = date("F j, Y", strtotime($fetch['complaint_date']));
						?>
							<tr>

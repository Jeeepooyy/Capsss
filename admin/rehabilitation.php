...
for ($month = 1; $month <= 12; $month++) {
	$query = $conn->query("SELECT COUNT(*) as total FROM `complaints` WHERE `section` = 'Rehabilitation' AND YEAR(`date`) = '$year' AND MONTH(`date`) = '$month'") or die(mysqli_error());
	$result = $query->fetch_array();
	$patient_counts[] = $result['total'];
}
// Fetch age distribution for Rehabilitation patients
$age_ranges = [
...
foreach ($age_ranges as $range => $condition) {
	$query = $conn->query("SELECT COUNT(DISTINCT complaints.itr_no) as total FROM `itr` INNER JOIN `complaints` ON itr.itr_no = complaints.itr_no WHERE complaints.section = 'Rehabilitation' AND YEAR(`date`) = '$year' AND $condition") or die(mysqli_error());
	$result = $query->fetch_array();
	$age_distribution[$range] = (int) $result['total'];
}

// Fetch gender distribution for Rehabilitation patients
$query = $conn->query("SELECT gender, COUNT(DISTINCT complaints.itr_no) as total FROM `itr` INNER JOIN `complaints` ON itr.itr_no = complaints.itr_no WHERE complaints.section = 'Rehabilitation' AND YEAR(`date`) = '$year' GROUP BY gender") or die(mysqli_error());
$gender_distribution = [];
...
if (array_sum($patient_counts) === 0) {
	$recommendations[] = "No patient records for Rehabilitation this year. Consider outreach programs to raise awareness.";
} else {
	...
	<script type="text/javascript">
		window.onload = function() {
			...
			$(".ageDistributionContainer").CanvasJSChart({
				...
				theme: "light2",
				title: {
					text: "Age Distribution of Rehabilitation Patients"
				},
				axisY: {
			...
			$(".genderDistributionContainer").CanvasJSChart({
				...
				theme: "light2",
				title: {
					text: "Gender Distribution of Rehabilitation Patients"
				},
				data: [{
					type: "pie",
					showInLegend: true,
				...
				<table id="table" cellspacing="0" class="display">
					<thead>
						<tr>
							...
							<th>Gender</th>
							<th>Address</th>
							<th>Complaint Date</th> <!-- New Column for Complaint Date -->
							<th>
					...
					<tbody>
						<?php
						// Fetch patients who have rehabilitation complaints
						$conn = new mysqli("localhost", "root", "", "hcpms") or die(mysqli_error());
						$query = $conn->query("SELECT itr.itr_no, itr.firstname, itr.lastname, itr.age, itr.gender, itr.address, complaints.date AS complaint_date FROM `complaints` INNER JOIN `itr` ON complaints.itr_no = itr.itr_no WHERE complaints.section = 'Rehabilitation' GROUP BY itr.itr_no ORDER BY complaints.date DESC") or die(mysqli_error());

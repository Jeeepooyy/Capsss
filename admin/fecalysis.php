...
for ($month = 1; $month <= 12; $month++) {
	$query = $conn->query("SELECT COUNT(DISTINCT complaints.itr_no) as total FROM `complaints` WHERE `section` = 'Fecalysis' AND YEAR(`date`) = '$year' AND MONTH(`date`) = '$month'") or die(mysqli_error());
	$result = $query->fetch_array();
	$patient_counts[] = (int) $result['total'];
}

// Fetch age distribution for Fecalysis patients
$age_ranges = [
...
foreach ($age_ranges as $range => $condition) {
	$query = $conn->query("SELECT COUNT(DISTINCT complaints.itr_no) as total FROM `itr` INNER JOIN `complaints` ON itr.itr_no = complaints.itr_no WHERE complaints.section = 'Fecalysis' AND $condition AND YEAR(complaints.date) = '$year'") or die(mysqli_error());
	$result = $query->fetch_array();
	$age_distribution[$range] = (int) $result['total'];
}

// Fetch gender distribution for Fecalysis patients
$query = $conn->query("SELECT gender, COUNT(DISTINCT complaints.itr_no) as total FROM `itr` INNER JOIN `complaints` ON itr.itr_no = complaints.itr_no WHERE complaints.section = 'Fecalysis' AND YEAR(complaints.date) = '$year' GROUP BY gender") or die(mysqli_error());
$gender_distribution = [];
...
if (array_sum($patient_counts) === 0) {
	$recommendations[] = "No patient records for Fecalysis this year. Consider outreach programs to raise awareness.";
} else {
...
<head>
	...
	<script type="text/javascript">
		window.onload = function() {
			...
			// Monthly patient population chart
			$(".chartContainer").CanvasJSChart({
				animationEnabled: true,
				theme: "light2",
				title: {
					text: "Monthly Fecalysis Patient Population " + year
				},
				axisY: {
			...
			$(".ageDistributionContainer").CanvasJSChart({
				...
				theme: "light2",
				title: {
					text: "Age Distribution of Fecalysis Patients"
				},
				axisY: {
			...
			$(".genderDistributionContainer").CanvasJSChart({
				...
				theme: "light2",
				title: {
					text: "Gender Distribution of Fecalysis Patients"
				},
				data: [{
					type: "pie",
					showInLegend: true,
		...
		<div class="container" style="padding-top: 80px;">
			...
			<div id="target">
				...
				<table id="table" class="display" cellspacing="0" width="100%">
					...
					<tbody>
						<?php
						// Fetch patients who have fecalysis complaints
						$conn = new mysqli("localhost", "root", "", "hcpms") or die(mysqli_error());
						$query = $conn->query("SELECT itr.itr_no, itr.firstname, itr.lastname, itr.age, itr.gender, itr.address, complaints.date AS complaint_date FROM complaints INNER JOIN itr ON complaints.itr_no = itr.itr_no WHERE complaints.section = 'Fecalysis' GROUP BY itr.itr_no ORDER BY complaints.date DESC") or die(mysqli_error());
